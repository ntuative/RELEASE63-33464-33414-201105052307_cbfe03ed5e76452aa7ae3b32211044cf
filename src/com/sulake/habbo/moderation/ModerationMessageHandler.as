package com.sulake.habbo.moderation
{
   ...
   public class ModerationMessageHandler
   {
      ...
      {
         if(param1 == null || this.var_65 == null)
         {
            return;
         }
         ...
         this.var_65.issueManager.playSound(_loc3_);
         this.var_65.issueManager.updateIssue(_loc3_);
      }
      ...
      private function onModeratorInit(param1:ModeratorInitMessageEvent) : void
      {
         ...
         {
            this.var_65.issueManager.updateIssue(_loc6_);
         }
         this.var_65.issueManager.updatePriorities();
         this.var_65.initMsg = _loc3_;
         this.var_65.startPanel.show();
      }
      ...
      private function onIssuePickFailed(param1:IssuePickFailedMessageEvent) : void
      {
         ...
         var pickedAlready:Boolean = this.var_65.issueManager.issuePickFailed(issues);
         if(pickedAlready)
         {
            if(retryEnabled)
            {
               ...
               {
                  alert = false;
                  this.var_65.issueManager.autoPick(retryEnabled,retryCount);
               }
            }
         }
         if(alert)
         {
            this.var_65.windowManager.alert("Error","Issue picking failed",0,function(param1:IAlertDialog, param2:WindowEvent):void
            {
               param1.dispose();
            });
         }
      ...
      private function onIssueDeleted(param1:IssueDeletedMessageEvent) : void
      {
         ...
         this.var_65.issueManager.removeIssue(_loc2_.issueId);
      }
      ...
      private function onUserInfo(param1:ModeratorUserInfoEvent) : void
      {
         ...
         for each(_loc3_ in this.var_1131)
         {
      ...
      private function onRoomInfo(param1:ModeratorRoomInfoEvent) : void
      {
         ...
         for each(_loc3_ in this.var_1130)
         {
      ...
      private function onCfhChatlog(param1:CfhChatlogEvent) : void
      {
         ...
         this.onChatlog("Call For Help Chatlog: " + _loc2_.data.chatRecordId,WindowTracker.const_1162,_loc2_.data.callId,_loc3_,_loc4_);
      }
      ...
      private function onRoomChatlog(param1:RoomChatlogEvent) : void
      {
         ...
         var _loc4_:Dictionary = new Dictionary();
         this.onChatlog("Room Chatlog: " + _loc2_.data.roomName,WindowTracker.const_717,_loc2_.data.roomId,_loc3_,_loc4_);
      }
      ...
      private function onUserChatlog(param1:UserChatlogEvent) : void
      {
         ...
         this.onChatlog("User Chatlog: " + _loc2_.data.userName,WindowTracker.const_1117,_loc2_.data.userId,_loc2_.data.rooms,_loc3_);
      }
      
      private function onChatlog(param1:String, param2:int, param3:int, param4:Array, param5:Dictionary) : void
      {
         var _loc6_:* = null;
         for each(_loc6_ in this.var_1132)
         {
      ...
      private function onRoomEnter(param1:RoomEntryInfoMessageEvent) : void
      {
         ...
         for each(_loc3_ in this.var_920)
         {
            _loc3_.onRoomChange();
         }
      }
      ...
      private function onRoomExit(param1:CloseConnectionMessageEvent) : void
      {
         ...
         for each(_loc2_ in this.var_920)
         {
            _loc2_.onRoomChange();
         }
      }
      ...
      private function onModeratorActionResult(param1:ModeratorActionResultMessageEvent) : void
      {
         ...
         if(_loc2_.success)
         {
            this.var_65.connection.send(new GetModeratorUserInfoMessageComposer(_loc2_.userId));
         }
         else
         {
            this.var_65.windowManager.alert("Alert","Moderation action failed",0,this.onAlertClose);
         }
      }
      ...
      public function removeChatlogListener(param1:ChatlogCtrl) : void
      {
         var _loc3_:* = null;
         var _loc2_:Array = new Array();
         for each(_loc3_ in this.var_1132)
         {
            if(_loc3_ != param1)
            {
               _loc2_.push(_loc3_);
