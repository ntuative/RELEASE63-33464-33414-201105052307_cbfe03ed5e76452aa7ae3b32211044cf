package com.sulake.habbo.moderation
{
   ...
   public class RoomToolCtrl implements IDisposable, TrackedWindow
   {
      ...
      private var var_1249:IDropMenuWindow;
      
      private var var_489:ITextFieldWindow;
      
      private var var_602:Boolean = true;
      ...
      public function show() : void
      {
         ...
         this.var_33.connection.send(new GetModeratorRoomInfoMessageComposer(this.var_367));
         Logger.log("BEGINNING TO SHOW: " + this.var_367);
      }
      ...
      {
         Logger.log("GOT ROOM INFO: " + param1.flatId + ", " + this.var_367);
         if(param1.flatId != this.var_367)
         {
            Logger.log("NOT THE SAME FLAT: " + param1.flatId + ", " + this.var_367);
            return;
         }
         this._data = param1;
         this.populate();
      ...
      public function populate() : void
      {
         ...
         this.var_489 = ITextFieldWindow(this._frame.findChildByName("message_input"));
         this.var_489.procedure = this.onInputClick;
         this.var_1249 = IDropMenuWindow(this._frame.findChildByName("msgTemplatesSelect"));
         this.prepareMsgSelect(this.var_1249);
         this.var_1249.procedure = this.onSelectTemplate;
         this.var_1077 = ICheckBoxWindow(this._frame.findChildByName("kick_check"));
      ...
      private function onChatlog(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_33.windowTracker.show(new ChatlogCtrl(new GetRoomChatlogMessageComposer(0,this._data.flatId),this.var_33,WindowTracker.const_717,this._data.flatId),this._frame,false,false,true);
      }
      ...
      {
         if(this.var_602 || this.var_489.text == "")
         {
            this.var_33.windowManager.alert("Alert","You must input a message to the user",0,this.onAlertClose);
            return;
         }
         var _loc2_:int = this.determineAction(param1,this.var_1077.isSelected);
         this.var_33.connection.send(new ModeratorActionMessageComposer(ModeratorActionMessageComposer.const_1841,_loc2_,this.var_489.text,"",""));
         if(this.var_1615.isSelected || this.var_1616.isSelected || this.var_1077.isSelected)
         {
            this.var_33.connection.send(new ModerateRoomMessageComposer(this._data.flatId,this.var_1615.isSelected,this.var_1616.isSelected,this.var_1077.isSelected));
         }
         this.dispose();
      }
      ...
      private function determineAction(param1:Boolean, param2:Boolean) : int
      {
         if(param2)
         {
            return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1563);
         }
         return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1619);
      }
      ...
      private function onSelectTemplate(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         var _loc3_:String = this.var_33.initMsg.roomMessageTemplates[this.var_1249.selection];
         if(_loc3_ != null)
         {
            this.var_602 = false;
            this.var_489.text = _loc3_;
         }
      }
   }
}
