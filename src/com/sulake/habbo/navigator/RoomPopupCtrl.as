package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2560 = new RoomThumbnailRenderer(param1);
         this.var_1262 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_181 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_181.visible = true;
         Util.hideChildren(this.var_181);
         this.refreshRoomName(this.var_181,this.var_118);
         if(this.var_118.ownerName != "" && this.var_118.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_181,!!this.var_118.event ? this.getEventCtg(this.var_118.categoryId) : this.getRoomCtg(this.var_118.categoryId));
         this.refreshRoomDesc(this.var_181,this.var_118);
         this.refreshExtraCont();
         this.refreshInfo(this.var_181,"trading_allowed",this.var_118.allowTrading);
         this.refreshInfo(this.var_181,"doormode_doorbell",this.var_118.doorMode == 1);
         this.refreshInfo(this.var_181,"doormode_password",this.var_118.doorMode == 2);
         Util.moveChildrenToColumn(this.var_181,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_181.height = Util.getLowestPoint(this.var_181);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_181,this.var_118.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_181.findChildByName("roomowner"));
         if(this.var_118.ownerName != "" && this.var_118.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_181.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1262.refreshTags(_loc1_,this.var_118.tags);
         ...
         this.var_2560.refreshThumbnail(_loc1_,this.var_118.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_266,Tabs.const_244,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
