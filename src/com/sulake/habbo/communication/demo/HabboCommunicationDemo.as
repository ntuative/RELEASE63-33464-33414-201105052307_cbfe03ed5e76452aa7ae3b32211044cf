package com.sulake.habbo.communication.demo
{
   ...
   public class HabboCommunicationDemo extends Component
   {
      ...
      private var var_543:IKeyExchange;
      
      private var var_1044:String;
      
      private var _view:HabboLoginDemoView;
      
      private var var_2784:PseudoRandom;
      ...
      private var var_235:HabboHotelView;
      
      private var var_1577:String = "";
      
      private const const_1904:String = "fuselogin";
      ...
      public function setSSOTicket(param1:String) : void
      {
         ...
         {
            this.var_674 = param1;
            this.var_56.initConnection(HabboConnectionType.const_284);
         }
      }
      ...
      private function onRoomSessionManagerReady(param1:IID, param2:IUnknown) : void
      {
         ...
         this._roomSessionManager.events.addEventListener(RoomSessionEvent.const_91,this.onRoomSessionEnded);
         this.checkRequirements();
      }
      ...
      {
         if(this.var_56 && this.var_139 && this._roomSessionManager && this._windowManager)
         {
      ...
      private function componentsAreRunning() : void
      {
         ...
         var _loc1_:Boolean = this.var_139.getBoolean("client.hotel_view.show_on_startup",true);
         this.var_235 = new HabboHotelView(this._windowManager,assets,Component(context).events,_loc1_);
         var _loc2_:String = this.var_139.getKey("client.hotel_view.image");
         var _loc3_:String = this.var_139.getKey("image.library.url");
         if(_loc2_ != null && _loc3_ != null)
         {
            this.var_235.loadHotelViewImage(_loc3_ + _loc2_);
         }
         var _loc4_:* = this.var_139.getKey("use.sso.ticket") == "1";
         this.var_674 = this.var_139.getKey("sso.ticket",null);
         this.var_2001 = this.var_139.getKey("flash.client.url","");
         this.var_2782 = this.var_139.getKey("external.variables.txt","");
         if(this.var_235 == null)
         {
            this.var_235 = new HabboHotelView(this._windowManager,assets,Component(context).events,_loc1_);
         }
         if(_loc4_)
         {
            this.var_56.mode = HabboConnectionType.const_95;
            if(this.var_674)
            {
               this.var_56.initConnection(HabboConnectionType.const_284);
            }
         ...
         else if(this._windowManager != null)
         {
            ...
            if(_loc5_)
            {
               ...
               this._view.addEventListener(HabboLoginDemoView.const_566,this.onInitConnection);
            }
            else
            {
      ...
      private function onInitConnection(param1:Event = null) : void
      {
         ...
         {
            this.var_56.mode = HabboConnectionType.const_95;
            if(this.var_139.keyExists("local.environment"))
            {
               this.var_56.mode = HabboConnectionType.const_1197;
            }
         }
         this.var_56.initConnection(HabboConnectionType.const_284);
      }
      ...
      private function onConnectionEstablished(param1:Event = null) : void
      {
         ...
         if(_loc2_ != null)
         {
            this.dispatchLoginStepEvent(HabboCommunicationEvent.const_944);
      ...
      private function onInitCrypto(param1:IMessageEvent) : void
      {
         ...
         this.var_2784 = new PseudoRandom(parseInt(_loc4_.substring(_loc4_.length - 4),16),65536);
         switch(this.var_56.mode)
         {
            case HabboConnectionType.const_95:
               _loc6_ = "";
               _loc6_ = this.var_139.getKey("url.prefix",_loc6_);
               _loc6_ = _loc6_.replace("http://","");
               _loc6_ = _loc6_.replace("https://","");
               _loc7_ = this.var_139.getKey("hotelview.banner.url","http:/\nsitename$/gamedata/banner");
               _loc7_ = _loc7_.replace("$sitename$",_loc6_);
               this.var_1577 = _loc4_;
               this.var_235.loadBannerImage(_loc7_ + "?token=" + this.var_1577,this.onHotelViewBannerLoaded);
               break;
            case HabboConnectionType.const_1197:
               this.sendConnectionParameters(_loc2_);
               break;
            case HabboConnectionType.const_1693:
               this.var_1044 = this.generateRandomHexString(30);
               this.var_543 = new DiffieHellman(new BigInteger(this.var_139.getKey("connection.development.prime"),16),new BigInteger(this.var_139.getKey("connection.development.generator"),16));
               this.var_543.init(this.var_1044);
               _loc8_ = this.var_543.getPublicKey(10);
      ...
      private function onSecretKeyEvent(param1:IMessageEvent) : void
      {
         ...
         this.var_543.generateSharedKey(_loc4_,10);
         var _loc5_:String = this.var_543.getSharedKey(16).toUpperCase();
         var _loc6_:RC4_R27 = new RC4_R27(null,null);
         var _loc7_:IEncryption = new RC4_R27(_loc6_,this.var_2784);
      ...
      private function sendConnectionParameters(param1:IConnection) : void
      {
         ...
         connection.send(new VersionCheckMessageComposer(401,this.var_2001,this.var_2782));
         var machineId:String = "";
         try
         {
            so = SharedObject.getLocal(this.const_1904,"/");
      ...
      private function onSessionParamsEvent(param1:IMessageEvent) : void
      {
         ...
         if(this.var_674 != null)
         {
            _loc4_ = new SSOTicketMessageComposer(this.var_674);
            _loc2_.send(_loc4_);
      ...
      private function onConnectionDisconnected(param1:Event) : void
      {
         ...
         if(this.var_1576)
         {
            this.dispatchLoginStepEvent(HabboCommunicationEvent.const_226);
         }
         if(param1.type == Event.CLOSE && !this.var_2783)
         {
            _loc2_ = this.var_139.getKey("logout.disconnect.url");
      ...
      private function onDisconnectReason(param1:DisconnectReasonEvent) : void
      {
         ...
         this.var_2783 = true;
         var _loc2_:String = this.var_139.getKey("logout.url");
         if(_loc2_.length > 0)
         {
      ...
      private function setOriginProperty(param1:String) : String
      {
         ...
         {
            return param1.replace("%origin%",this.var_139.getKey("flash.client.origin","popup"));
         }
         return param1;
      }
      ...
      private function onHotelViewBannerLoaded(param1:BitmapData) : void
      {
         ...
         var _loc4_:String = xor(_loc3_,this.var_1577);
         ...
         this.var_543 = new DiffieHellman(_loc10_,_loc11_);
         var _loc13_:int = 10;
         var _loc14_:* = null;
         while(_loc13_ > 0)
         {
            _loc14_ = this.generateRandomHexString(30);
            this.var_543.init(_loc14_);
            _loc15_ = this.var_543.getPublicKey(10);
            if(_loc15_.length >= 64)
            {
               _loc12_ = _loc15_;
               this.var_1044 = _loc14_;
            }
