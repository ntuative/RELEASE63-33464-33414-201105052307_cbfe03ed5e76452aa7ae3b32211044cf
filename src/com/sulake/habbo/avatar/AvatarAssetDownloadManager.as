package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_957:int = 3;
      
      private var var_436:AssetLoaderStruct;
      
      private var var_958:Timer;
      ...
      private const const_2142:int = 100;
      
      private const const_1678:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_436 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_958 = new Timer(this.const_2142,1);
         this.var_958.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_436)
         {
            this.var_436.addEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_436.addEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_436)
         {
            this.var_436.removeEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_436.removeEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_957);
            if(this.var_1448.indexOf("?") > 0)
            {
               _loc2_ = this.var_1448 + "&retry=" + this.var_957;
            }
            else
            {
               _loc2_ = this.var_1448 + "?retry=" + this.var_957;
            ...
            this.var_436 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2471,this._assets);
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_570[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_570[_loc3_] = _loc6_;
            }
            this.var_959[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_959)
         {
            _loc7_ = true;
            _loc4_ = this.var_959[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete this.var_570[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_959[_loc3_];
         }
         while(_loc6_ < this.var_772.length)
         {
            _loc5_ = this.var_772[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_772.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_958.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_773.length > 0 && this.var_772.length < this.const_1678)
         {
            _loc1_ = this.var_773[0];
            _loc1_.startDownloading();
            this.var_772.push(this.var_773.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_773.indexOf(param1) == -1 && this.var_772.indexOf(param1) == -1)
         {
