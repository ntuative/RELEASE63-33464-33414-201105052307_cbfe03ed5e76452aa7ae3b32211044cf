package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_848:IFigurePartSet;
      ...
      private var var_685:Rectangle;
      
      private var var_2056:int = 0;
      
      private var var_465:IAvatarRenderManager;
      
      private var var_1088:Boolean;
      
      private var var_2058:BitmapData;
      
      private const const_2159:int = 13421772;
      
      private const const_2160:int = 16777215;
      
      private const const_375:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_848 = param3;
         this._window = param1;
         this.var_1550 = this._window.findChildByTag("BG_COLOR");
         this._colors = param4;
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_2056 = Math.max(this.var_2056,_loc6_.colorLayerIndex);
            }
         }
         this.var_465 = this.var_66.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_707 != null && !this.var_2057)
            {
               _loc3_ = this.var_707;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_848)
            {
               switch(this.var_848.clubLevel)
               {
                  case HabboClubLevelEnum.const_55:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_45:
                     _loc2_.style = HabboIconType.const_952;
                     break;
                  case HabboClubLevelEnum.const_47:
                     _loc2_.style = HabboIconType.const_777;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1550.color = this.const_2159;
         }
         else
         {
            this.var_1550.color = this.const_2160;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_465.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_465.isFigureReady(_loc1_))
         {
            this.var_465.method_12(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_375[_loc6_] + "_" + FigureData.const_150;
               _loc4_ = this.var_465.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_375.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_375[_loc6_] + "_" + FigureData.const_150;
                  _loc4_ = this.var_465.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_685 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_2058)
               {
                  _loc9_ = this.var_66.controller.assets.getAssetByName("avatar_editor_download_icon");
                  this.var_2058 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_685.width,this.var_685.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_685.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_685.y;
               if(this.var_2057 && _loc2_.colorLayerIndex > 0)
               {
