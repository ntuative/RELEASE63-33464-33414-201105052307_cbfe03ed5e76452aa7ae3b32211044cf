package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridColorItem
   {
      ...
      private var var_1048:IPartColor;
      ...
      private function updateThumbData() : void
      {
         ...
         if(_loc2_ != null)
         {
            _loc2_.bitmap = new BitmapData(this.var_1033.width,this.var_1033.height,true,0);
            _loc5_ = this.var_1033.clone();
            _loc5_.colorTransform(_loc5_.rect,this.var_1048.colorTransform);
         ...
         if(_loc3_ != null)
         {
            if(this._isSelected)
            {
               _loc3_.bitmap = new BitmapData(this.var_1031.width,this.var_1031.height,true,16777215);
               _loc3_.bitmap.copyPixels(this.var_1031,this.var_1031.rect,new Point(0,0),null,null,true);
               _loc3_.visible = true;
            }
            else
            {
         ...
         if(_loc4_)
         {
            if(this.var_1048)
            {
               switch(this.var_1048.clubLevel)
               {
                  case HabboClubLevelEnum.const_55:
                     _loc4_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_45:
                     _loc4_.style = HabboIconType.const_952;
                     _loc4_.visible = true;
                     break;
                  case HabboClubLevelEnum.const_47:
                     _loc4_.style = HabboIconType.const_777;
                     _loc4_.visible = true;
               }
            }
            else
