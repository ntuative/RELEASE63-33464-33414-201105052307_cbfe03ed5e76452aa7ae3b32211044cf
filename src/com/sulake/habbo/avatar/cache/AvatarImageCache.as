package com.sulake.habbo.avatar.cache
{
   ...
   public class AvatarImageCache
   {
      ...
      private static const const_1910:int = 60000;
      ...
      private var var_419:AvatarImageBodyPartContainer;
      
      private var var_1602:String;
      
      private var var_124:AvatarImagePartContainer;
      
      private var var_2845:Point;
      ...
      private var var_2844:ByteArray;
      
      private var var_2846:ImageData;
      
      private var var_1007:PaletteMap;
      ...
      public function dispose() : void
      {
         ...
         {
            _loc2_ = this.var_281.getKeys();
            for each(_loc3_ in _loc2_)
            {
               _loc1_ = this.var_281.getValue(_loc3_) as AvatarImageBodyPartCache;
               if(_loc1_ != null)
               {
                  _loc1_.dispose();
               }
      ...
      public function disposeInactiveActions(param1:int = 60000) : void
      {
         ...
         var _loc4_:Array = this.var_281.getKeys();
         for each(_loc5_ in _loc4_)
         {
            _loc3_ = this.var_281.getValue(_loc5_) as AvatarImageBodyPartCache;
            if(_loc3_ != null)
            {
      ...
      public function resetBodyPartCache(param1:IActiveActionData) : void
      {
         var _loc2_:* = null;
         var _loc4_:* = null;
         var _loc3_:Array = this.var_281.getKeys();
         for each(_loc4_ in _loc3_)
         {
            _loc2_ = this.var_281.getValue(_loc4_) as AvatarImageBodyPartCache;
            if(_loc2_ != null)
            {
      ...
      public function setGeometryType(param1:String) : void
      {
         ...
         if(this.var_849 == GeometryType.const_1153 && param1 == GeometryType.const_1164 || this.var_849 == GeometryType.const_1164 && param1 == GeometryType.const_1153)
         {
            this.var_849 = param1;
            this.var_87 = null;
            return;
      ...
      public function getImageContainer(param1:String, param2:int) : AvatarImageBodyPartContainer
      {
         ...
         if(this._bodyPartCache == null)
         {
            ...
            this.var_281.add(param1,this._bodyPartCache);
         }
            ...
            if(this.var_397.definition.isAnimation)
            {
               ...
               if(_loc12_ != null)
               {
                  ...
                  if(_loc14_ != null)
                  {
                     ...
                     if(_loc14_.type == AnimationLayerData.const_721)
                     {
                        if(_loc14_.action != null)
                        {
                     ...
                     else if(_loc14_.type == AnimationLayerData.const_619)
                     {
                        _loc3_ = _loc11_;
                     }
         ...
         if(_loc10_ == null)
         {
            _loc15_ = this._structure.getParts(param1,this.var_1402.getFigure(),_loc5_,this.var_849,_loc3_,_loc6_,_loc7_);
            _loc10_ = new AvatarImageDirectionCache(_loc15_);
            _loc9_.updateDirectionCache(_loc3_,_loc10_);
         }
         this.var_419 = _loc10_.getImageContainer(_loc4_);
         if(this.var_419 == null)
         {
            _loc16_ = _loc10_.getPartList();
            this.var_419 = this.renderBodyPart(_loc3_,_loc16_,_loc4_,this.var_397);
            if(this.var_419 == null)
            {
               return null;
            }
            _loc10_.updateImageContainer(this.var_419,_loc4_);
         }
         this.var_419.offset = _loc8_;
         return this.var_419;
      }
      
      public function getBodyPartCache(param1:String) : AvatarImageBodyPartCache
      {
         var _loc2_:AvatarImageBodyPartCache = this.var_281.getValue(param1) as AvatarImageBodyPartCache;
         if(_loc2_ == null)
         {
            _loc2_ = new AvatarImageBodyPartCache();
            this.var_281.add(param1,_loc2_);
         }
         return _loc2_;
      }
      ...
      private function renderBodyPart(param1:int, param2:Array, param3:int, param4:IActiveActionData) : AvatarImageBodyPartContainer
      {
         ...
         if(!this.var_87)
         {
            this.var_87 = this._structure.getCanvas(this._scale,this.var_849);
            if(!this.var_87)
            {
               return null;
            }
            this.var_2845 = new Point(0,this.var_87.height - int(this.var_87.width / 4));
         }
         var _loc5_:int = param1;
         var _loc6_:Boolean = AvatarDirectionAngle.const_1649[param1];
         var _loc9_:String = param4.definition.assetPartDefinition;
         var _loc10_:String = param4.definition.state;
         var _loc11_:Boolean = _loc10_ == AvatarAction.const_778 || _loc10_ == AvatarAction.const_448;
         ...
         while(_loc8_ >= 0)
         {
            ...
            {
               this.var_1602 = this.var_124.bodyPartId;
               this.var_191 = this.var_124.partType;
               this.var_2059 = this.var_124.partId;
               ...
               if(_loc6_)
               {
                  ...
                  else
                  {
                     ...
                     if(this.var_124.flippedPartType != this.var_191)
                     {
                        this.var_191 = this.var_124.flippedPartType;
                     }
                  }
               }
               this.var_1601 = this._scale + "_" + this.var_124.action.assetPartDefinition + "_" + this.var_191 + "_" + this.var_2059 + "_" + _loc5_ + "_" + this._frame;
               this.var_466 = this._assets.getAssetByName(this.var_1601) as BitmapDataAsset;
               if(this.var_466 == null)
               {
                  this.var_1601 = this._scale + "_std_" + this.var_191 + "_" + this.var_2059 + "_" + _loc5_ + "_0";
                  this.var_466 = this._assets.getAssetByName(this.var_1601) as BitmapDataAsset;
               }
               if(!(!this.var_466 || !this.var_466.content))
               {
                  if(this.var_124.paletteMapId > -1)
                  {
                     this.var_1007 = this._structure.renderManager.paletteManager.getPalette(this.var_124.paletteMapId);
                     if(this.var_1007 != null)
                     {
                        this.var_1063 = this.var_466.content as BitmapData;
                        this.var_192 = new BitmapData(this.var_1063.width,this.var_1063.height);
                        this.var_2844 = this.var_1007.colorize(this.var_1063.getPixels(this.var_1063.rect));
                        this.var_192.setPixels(this.var_192.rect,this.var_2844);
                     }
                     else
                     {
                        this.var_192 = (this.var_466.content as BitmapData).clone();
                     }
                  }
                  else
                  {
                  ...
                  if(_loc11_ && (this.var_1602 == AvatarBodyPartType.const_283 || this.var_1602 == AvatarBodyPartType.const_1710 || this.var_1602 == AvatarBodyPartType.const_1806) && this.var_124.color != null)
                  {
                     _loc16_ = 0.7;
                  ...
                  {
                     this.var_192.colorTransform(this.var_192.rect,this.var_124.color.colorTransform);
                  }
                  if(this.var_124.isBlendable)
                  {
                     this.var_192.colorTransform(this.var_192.rect,this.var_124.blendTransform);
                  }
                  this.var_1061 = this.var_466.offset.clone();
                  if(_loc7_)
                  {
                     if(this._scale == AvatarScaleType.const_56)
                     {
                  ...
                  this.var_2846 = new ImageData(this.var_192,this.var_1061,_loc7_);
                  this.var_1062.push(this.var_2846);
               }
            }
            _loc8_--;
         }
         ...
         var _loc13_:ImageData = this.createUnionImage(this.var_1062,_loc6_);
         var _loc14_:Point = new Point(-1 * _loc13_.regpoint.x,this.var_2845.y - _loc13_.regpoint.y);
         if(_loc6_)
         {
            if(this._scale == AvatarScaleType.const_56)
            {
         ...
         _loc8_ = this.var_1062.length - 1;
         while(_loc8_ >= 0)
         {
            _loc15_ = this.var_1062.pop();
            if(_loc15_)
            {
               _loc15_.dispose();
            }
