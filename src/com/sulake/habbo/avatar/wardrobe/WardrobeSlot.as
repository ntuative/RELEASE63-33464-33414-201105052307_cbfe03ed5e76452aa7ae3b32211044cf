package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private var var_375:IBitmapWrapperWindow;
      ...
      private function createView() : void
      {
         ...
         this.var_375 = this._view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(this.var_641 && this.var_1506)
         {
            _loc5_ = this._controller.avatarRenderManager.createAvatarImage(this.figure,AvatarScaleType.const_93,this.var_993,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_35,parseInt(FigureData.const_832));
         ...
         if(this.var_375)
         {
            if(this.var_375.bitmap)
            {
               this.var_375.bitmap.dispose();
            }
            this.var_375.bitmap = new BitmapData(this.var_375.width,this.var_375.height,true,0);
            _loc7_ = (this.var_375.width - _loc1_.width) / 2;
            _loc8_ = (this.var_375.height - _loc1_.height) / 2;
            this.var_375.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "set_button":
               this.var_641 = this._controller.figureData.getFigureString();
               this.var_993 = this._controller.gender;
               this._controller.handler.saveWardrobeOutfit(this.var_1878,this);
               this.updateView();
               break;
            case "get_button":
               if(this.var_641)
               {
                  this._controller.loadAvatarInEditor(this.var_641,this.var_993,this._controller.clubMemberLevel);
               }
         }
      }
