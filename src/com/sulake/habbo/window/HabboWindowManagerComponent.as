package com.sulake.habbo.window
{
   ...
   public class HabboWindowManagerComponent extends Component implements IHabboWindowManager, ICoreWindowManager, IWindowFactory, IUpdateReceiver, IWindowContextStateListener
   {
      ...
      private static const const_462:uint = 3;
      
      private static const const_1457:uint = 1;
      ...
      private var var_1501:IEventDispatcher;
      
      private var var_1223:Boolean = false;
      
      private var var_1873:ProfilerOutput;
      
      private var var_1499:ProfilerAgentTask;
      
      private var var_1500:ProfilerAgentTask;
      ...
      private function initialize(param1:IID, param2:IUnknown) : void
      {
         ...
         this.var_948 = new SkinContainer(_loc4_.content as XML,assets);
         this._windowRenderer = new WindowRenderer(this.var_948);
         this.var_207 = new Array(const_462);
         ...
         var _loc6_:int = 0;
         while(_loc6_ < const_462)
         {
            this.var_207[_loc6_] = new WindowContext("layer_" + _loc6_,this._windowRenderer,this,this._localization,context.displayObjectContainer,_loc5_,this);
            _loc6_++;
         }
         this.var_270 = this.var_207[const_1457];
         unlock();
      ...
      private function receiveProfilerInterface(param1:IID, param2:IUnknown) : void
      {
         ...
         if(_loc3_ != null)
         {
            if(!this.var_1873)
            {
               this.var_1873 = new ProfilerOutput(context,this,this._windowRenderer);
            }
            this.var_1873.profiler = _loc3_;
            this.var_1499 = new ProfilerAgentTask("Update","Event processing");
            _loc3_.getProfilerAgentForReceiver(this).addSubTask(this.var_1499);
            this.var_1500 = new ProfilerAgentTask("Redraw","Window rasterizer");
            _loc3_.getProfilerAgentForReceiver(this).addSubTask(this.var_1500);
            this.var_1223 = true;
         }
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            ...
            if(this.var_207)
            {
               while(this.var_207.length > 0)
               {
                  IDisposable(this.var_207.pop()).dispose();
               }
            }
            this.var_207 = null;
            if(this.var_267)
      ...
      {
         var _loc3_:Dictionary = this.var_267[param2] as Dictionary;
         if(_loc3_ == null)
         {
            return null;
         }
         var _loc4_:XML = _loc3_[param1] as XML;
         return _loc4_ == null ? this.var_267[0][param1] as XML : _loc4_;
      }
      
      public function getDefaultsByTypeAndStyle(param1:uint, param2:uint) : DefaultAttStruct
      {
         return this.var_948.getDefaultAttributesByTypeAndStyle(param1,param2);
      }
      ...
      {
         return this.var_207[param9].create(param1,param2,param3,param4,param5,param6,param7,null,param8,null,null);
      }
      
      public function removeWindow(param1:String, param2:uint = 1) : void
      {
         var _loc3_:IDesktopWindow = this.var_207[param2].getDesktopWindow();
      ...
      {
         return this.var_207[param2].getDesktopWindow().getChildByName(param1);
      }
      
      public function getActiveWindow(param1:uint = 1) : IWindow
      {
         return this.var_207[param1].getDesktopWindow().getChildAt(this.var_270.getDesktopWindow().numChildren - 1);
      }
      ...
      {
         var _loc2_:IWindowContext = this.var_207[param1];
      ...
      public function update(param1:uint) : void
      {
         ...
         if(WindowContext.var_402.length > 0)
         {
            this.var_1501.dispatchEvent(new Event(HabboWindowTrackingEvent.HABBO_WINDOW_TRACKING_EVENT_INPUT));
            _loc2_ = const_462 - 1;
            while(_loc2_ >= 0)
            {
               this.var_207[_loc2_].update(param1);
               _loc2_--;
            }
         }
         if(this.var_1223)
         ...
         this.var_1501.dispatchEvent(new Event(HabboWindowTrackingEvent.HABBO_WINDOW_TRACKING_EVENT_RENDER));
         _loc2_ = 0;
         while(_loc2_ < const_462)
         {
            this.var_207[_loc2_].render(param1);
            _loc2_++;
         }
         if(this.var_1223)
         {
         ...
         if(WindowContext.var_402.length > 0)
         {
            WindowContext.var_402.flush();
         }
         MouseCursorControl.change();
         this.var_1501.dispatchEvent(new Event(HabboWindowTrackingEvent.HABBO_WINDOW_TRACKING_EVENT_SLEEP));
      }
      ...
      public function mouseEventReceived(param1:String, param2:IWindow) : void
      {
         if(param2 != null)
         {
            ...
            {
               ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1825,new Date().getTime().toString());
               ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1764,param2.name + ": " + param2.toString());
            }
            else if(param1 == MouseEvent.MOUSE_UP)
            {
               ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1709,new Date().getTime().toString());
               ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1724,param2.name + ": " + param2.toString());
            }
         }
      }
      ...
      private function performTestCases() : void
      {
         ...
         _loc1_.begin(this);
         _loc1_.test({
            "type":WindowType.const_567,
            "style":WindowStyle.const_132,
            "caption":"Testing",
            "count":800,
            "properties":[_loc2_]
         });
