package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_1320:Number = 1.0;
      
      private var var_686:BitmapData;
      
      private var var_851:ColorTransform;
      
      private var var_1603:ColorTransform;
      
      private var var_2063:Matrix;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_851 = new ColorTransform();
         this.var_851.alphaMultiplier = 1;
         this.var_1603 = new ColorTransform();
         this.var_2063 = new Matrix();
      ...
      {
         var _loc1_:* = null;
         for each(_loc1_ in this.var_1319)
         {
            _loc1_.dispose();
         }
         this.var_1319 = null;
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_99 = this.var_1319[param1];
         this.var_1067 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_2062 >= 0)
         {
            this.var_282 = this._visualization.getSprite(this.var_2062);
            if(this.var_282 && this.var_282.asset)
            {
               ...
               if(this.var_87 == null)
               {
                  this.var_87 = this.var_282.asset.clone();
                  if(this.var_851.alphaMultiplier != 1)
                  {
                     this.var_686 = new BitmapData(this.var_87.width,this.var_87.height,true,4278190080);
                  }
               }
               this.var_2587 = -this.var_282.offsetX;
               this.var_2588 = -this.var_282.offsetY;
               this.var_282.asset = this.var_87;
            }
            if(this.var_87)
            {
            ...
            if(this.var_686)
            {
               this.var_686.fillRect(this.var_686.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_99.y * this.var_1320;
         }
         return 0;
      }
      ...
      {
         if(!this.var_99 || !this.var_282)
         {
            return;
         }
         if(this.var_87 && this.var_282.asset != this.var_87)
         {
            this.var_282.asset = this.var_87;
         }
         if(this.var_1067)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_1067 && this.var_99.hasIgnited)
         {
            this.var_1067 = true;
         }
         _loc8_ = this.var_1147 * this.var_1320;
         this.var_99.update();
         if(this.var_1067)
         {
            if(this.var_99.roomObjectSpriteId >= 0)
            ...
            {
               this.var_87.draw(this.var_686,this.var_2063,this.var_851,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_99.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2587 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1320;
               _loc4_ = this.var_2588 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1320;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1603.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_87.draw(_loc7_,this._translationMatrix,this.var_1603,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2847 = parseInt(param1.@size);
         this.var_2062 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1147 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1320 = this.var_2847 / 64;
         this._blend = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this._blend = Math.min(this._blend,1);
         this.var_851.alphaMultiplier = this._blend;
