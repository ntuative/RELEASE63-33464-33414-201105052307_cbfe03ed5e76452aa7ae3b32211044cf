package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1450:String = "asset-libraries";
      
      private static const const_1447:String = "library";
      
      private static const const_1446:String = "service-libraries";
      
      private static const const_1448:String = "library";
      
      private static const const_1445:String = "component-libraries";
      
      private static const const_1451:String = "library";
      
      private static const const_1452:String = "error_data";
      
      private static const const_1449:String = "error_cat";
      
      private static const const_1453:String = "error_desc";
       
      
      private var var_371:LibraryLoaderQueue;
      
      private var var_786:IEventDispatcher;
      
      private var var_519:uint;
      
      private var var_370:Function;
      
      private var var_205:Profiler;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         {
            this.var_263.push(new Array());
            this.var_633.push(0);
            _loc3_++;
         }
         this._lastUpdateTimeMs = getTimer();
         attachComponent(this,[new IIDCore()]);
         var_404.addEventListener(Event.ENTER_FRAME,this.onEnterFrame);
         switch(param2 & 0)
         {
            case Core.const_1208:
               debug("Core; using simple frame update handler");
               this.var_370 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_919:
               debug("Core; using complex frame update handler");
               this.var_370 = this.complexFrameUpdateHandler;
               break;
            case Core.const_883:
               debug("Core; using profiler frame update handler");
               this.var_370 = this.profilerFrameUpdateHandler;
               this.var_205 = new Profiler(this);
               attachComponent(this.var_205,[new IIDProfiler()]);
               var_404.addChild(new ProfilerViewer(this.var_205));
               break;
            case Core.const_546:
               debug("Core; using experimental frame update handler");
               this.var_370 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_370 = this.debugFrameUpdateHandler;
         }
      }
      
      public function getNumberOfFilesPending() : uint
      {
         return this.var_371.length;
      }
      
      public function getNumberOfFilesLoaded() : uint
      {
         return this.var_519 - this.getNumberOfFilesPending();
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            try
            {
               ...
               {
                  receivers = this.var_263[i] as Array;
            ...
            if(var_404)
            {
               var_404.removeEventListener(Event.ENTER_FRAME,this.onEnterFrame);
               var_404 = null;
            }
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1452,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1449,String(param3));
         ErrorReportStorage.setParameter(const_1453,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_786 = loadingEventDelegate;
         if(this.var_371 == null)
         {
            this.var_371 = new LibraryLoaderQueue(_debug);
         }
         try
         {
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_318,e);
         }
         try
         {
            node = config.child(const_1446)[0];
            if(node != null)
            {
               list = node.child(const_1448);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_318,e);
         }
         try
         {
            node = config.child(const_1445)[0];
            if(node != null)
            {
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_318,e);
         }
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_786)
         {
            this.var_786.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_786)
         {
            ...
            this.var_786.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_786)
                  {
                     this.var_786.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_519 - this.var_371.length,this.var_519,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_205 ? 0 : this.var_1486 & 0;
         if(_loc3_ == Core.const_546)
         {
            this.var_263[param2].push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            this.var_263[param2].push(param1);
         }
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc3_:* = null;
         var _loc6_:* = null;
         var _loc4_:int = !!this.var_205 ? 0 : this.var_1486 & 0;
         var _loc5_:int = 0;
         while(_loc5_ < CoreComponentContext.const_109)
         {
            _loc3_ = this.var_263[_loc5_] as Array;
            if(_loc4_ == Core.const_546)
            {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_370(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(priority < CoreComponentContext.const_109)
         {
            ...
            receivers = this.var_263[priority];
            index = 0;
            length = receivers.length;
            while(index != length)
            {
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         var maxTimePerFrame:uint = 1000 / var_404.stage.frameRate;
         priority = 0;
         while(priority < CoreComponentContext.const_109)
         {
            t = getTimer();
         ...
         while(priority < CoreComponentContext.const_109)
         {
            ...
            while(index != length)
            {
               ...
               else
               {
                  try
                  {
                     this.var_205.update(receiver,msSinceLastUpdate);
                  }
                  catch(e:Error)
                  {
                     error("Error in update receiver \"" + getQualifiedClassName(receiver) + "\": " + e.message,true,e.errorID,e);
      ...
      public function setProfilerMode(param1:Boolean) : void
      {
         ...
         else
         {
            if(this.var_205)
            {
               detachComponent(this.var_205);
               this.var_205.dispose();
               this.var_205 = null;
            }
            switch(this.var_1486 & 0)
            {
               case Core.const_1208:
                  this.var_370 = this.simpleFrameUpdateHandler;
                  break;
